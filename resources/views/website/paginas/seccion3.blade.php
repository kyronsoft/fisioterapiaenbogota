<section class="container-fluid d-flex justify-content-start seccion-cuatro">
    <div class="col-xs-5 col-md-6 align-self-center bg-white columna">
        <div class="text-center">@php echo $inicioseccion3[0]->spanish; @endphp</div>
        <div class="row">
            <div class="col-xs-2 col-md-1 align-self-center">
                <img src="images/icon-decor-1.png" alt="" class="rounded-circle img-sec3" style="background-color: #ddb2c6;">
            </div>
            <div class="col-xs-10 col-md-11">
                <div class="texto-sec4">@php echo $inicioseccion3[1]->spanish; @endphp</div>
            </div>
        </div>
        <div class="row">
            <div class="col-xs-2 col-md-1 align-self-center">
                <img src="images/icon-decor-2.png" alt="" class="rounded-circle img-sec3" style="background-color: #ddad9f;">
            </div>
            <div class="col-xs-10 col-md-11">
                <div class="texto-sec4">@php echo $inicioseccion3[2]->spanish; @endphp</div>
            </div>
        </div>
        <div class="row">
            <div class="col-xs-2 col-md-1 align-self-center">
                <img src="images/icon-decor-3.png" alt="" class="rounded-circle img-sec3" style="background-color: #b4c0ca;">
            </div>
            <div class="col-xs-10 col-md-11">
                <div class="texto-sec4">@php echo $inicioseccion3[3]->spanish; @endphp</div>
            </div>
        </div>
    </div>
</section>
